...
/**
 * Cocos MCP (3.8.x) — single-file extension, no npm dependencies.
 *
 * Contents (top → bottom):
 *   1. Minimal WebSocket client (RFC 6455, text frames only, built on
 *      Node's built-in `net` + `crypto` — no `ws` package).
 *   2. Console ring buffer for read_console.
 *   3. The three command handlers Python invokes: get_project_info,
 *      read_console, manage_scene.
 *   4. Extension lifecycle (load/unload) + panel-facing methods.
 *
 * Protocol (server → us): { id, command, params }
 *          (us → server): { id, success, data }
 *                     or  { id, success: false, error, stack? }
 */
...
// ----------------------------------------------------------------------- //
//
// Why not use the `ws` npm package? So this folder is one-paste / zero-
// install. Cocos Creator 3.8.x ships an Electron with Node 18/20 which
// doesn't have a built-in WebSocket client, so we hand-roll one. ~120
// lines, text frames only, masks outgoing payloads (per RFC), responds
// to ping/close, and tops out at 16MB frames.
...
class WsClient extends EventEmitter {
    ...
    _onData(chunk) {
        ...
        // Parse zero or more frames out of _buf
        while (true) {
            ...
            // other opcodes (binary, pong, continuation) ignored
        }
    }
    ...
    _writeFrame(opcode, data) {
        ...
        header.copy(out, 0);
        for (let i = 0; i < len; i++) {
            out[header.length + i] = data[i] ^ maskKey[i & 3];
        }
...
function _installConsoleHook() {
    ...
    ['log', 'info', 'warn', 'error'].forEach((level) => {
        ...
        _consoleOriginals[level] = console[level];
...
const handlers = {
    ...
    async get_project_info(/* params */) {
        ...
        const editorVersion = _safe(() => {
            if (Editor.App && Editor.App.version) return Editor.App.version;
        ...
        return {
            ...
            firstScenes: scenes.slice(0, 20).map((s) => ({
                url:  s.source || s.url || null,
                uuid: s.uuid,
                path: s.file || s.path || null,
            })),
    ...
    async manage_scene(params) {
        ...
        if (action === 'current') {
            let info = null;
            try { info = await Editor.Message.request('scene', 'query-current-scene'); }
            catch (e) { info = null; }
            if (info && info.uuid) {
                let childCount;
                try {
                    const tree = await Editor.Message.request('scene', 'query-node-tree', info.uuid);
                    if (tree && tree.children) childCount = tree.children.length;
            ...
            // Fallback: ask for node tree without uuid → some patch levels return scene root
            let tree = null;
            try { tree = await Editor.Message.request('scene', 'query-node-tree'); }
            catch (e) { tree = null; }
            if (!tree || !tree.uuid) return { hasScene: false };
            return {
                hasScene: true,
                uuid: tree.uuid,
                name: tree.name,
        ...
        if (action === 'open') {
            ...
            if (!uuid && params.url) {
                uuid = await Editor.Message.request('asset-db', 'query-uuid', _normUrl(params.url));
            }
...
exports.load = function load() {
    _installConsoleHook();
    console.log('[' + PACKAGE_NAME + '] loaded — open the panel and click Connect.');
};
...
exports.methods = {
    async openPanel() {
        await Editor.Panel.open(PACKAGE_NAME + '.default');
    },
    panelStatus() {
        return _connectionState();
    },
    async panelConnect(url) {
        // _connect always resolves with the current state; never rejects.
